...
export const Assessment = ({ selectedVideo, statusOfInstance, type }) => {
  ...
  const handleGetAssessment = async (selectedVideo) => {
    ...
    try {
      ...
      const url =
        "https://j24m9na7zc.execute-api.ap-south-1.amazonaws.com/dev/";
  ...
  return (
    <div className="bg-white self-start h-auto lg:w-[40%] w-full flex flex-col p-5 lg:m-5 my-5 rounded-lg shadow-md">
      ...
      <div
        className={`${
          ...
          `border border-[#e2e8f0] border-opacity-20  p-4 flex  my-2  pb-4  ${
            type === "doc" ? "max-h-[160px] overflow-y-auto" : ""
          }`
        }  rounded-lg  ${loader && "border-none"}`}
      >
        {loader && <Loader prompt="Generating Assessment" />}
        {errorMessage ? (
...
const QuestionAnswer = ({
  ...
  return (
    index + 1 === currentPage && (
      <div
        ref={chatAreaRef}
        className="min-w-full grow max-h-[300px] overflow-y-scroll custom-scrollbar-assess"
      >
        <p className="text-primary text-base p-2">{question}</p>
        {type === "MCQ" && (
