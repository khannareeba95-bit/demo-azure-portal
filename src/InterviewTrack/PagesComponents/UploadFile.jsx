...
import { fetchFilePresignedUrl, uploadFileToS3WithUrl } from "../utils/ApiCall";
...
const UploadFile = ({
  ...
  const handleUploadAll = async () => {
    ...
    try {
      const uploadPromises = selectedFiles.map(async (file) => {
        const presignedUrl = await fetchFilePresignedUrl(file.name);
        if (presignedUrl) {
  ...
  const renderFileList = () => (
    <div className="space-y-2 mt-2 max-h-[300px] overflow-y-scroll h-full">
  ...
  const renderUploadedFiles = () => (
    <div className="space-y-2 my-2 overflow-y-auto max-h-[200px] h-full">
        ...
        {selectedFiles && selectedFiles.length > 0 && (
          <>
            <h2 className="font-semibold text-lg flex items-center font-sans text-amber-600">
              ...
              />
              Selected Files ({selectedFiles.length} files selected)
            </h2>
            {renderFileList()}
