...
export const Summary = ({
  ...
  const handleGetSummary = async (video) => {
    ...
    try {
      ...
      const url = buildApiUrl(selectedOption?.value);
  ...
  const buildPayload = (video) => {
    if (selectedOption.value === "math") {
      return { transcript: video?.transcription, source: "summary" };
    }
    if (type === "doc") {
      return { transcript: video?.transcription, source: "documents" };
    }
    return selectedOption?.value !== "en"
      ? { transcript: video?.transcriptJson, flag: selectedOption?.value }
  ...
  return (
    <div
      className={`${
        type === "doc" ? "lg:w-[50%] xl:w-[55%] 2xl:w-[55%]" : "lg:w-[60%]"
      } w-full ${
        selectedOption === "math" ? "max-h-[800px] xl:max-h-[600px]" : "h-auto"
      } self-start overflow-y-scroll bg-white rounded-lg p-5 lg:m-5 my-5 shadow-md`}
    >
      <div
      ...
      >
        <h1 className="text-primary lg:text-xl text-lg p-2">
          Summary of the {type === "doc" ? "PDF" : "video"}
        </h1>
      ...
      {summary ? (
        <>
          ...
          {selectedOption.value === "math" &&
            ...
            summary?.explain?.length &&
            summary?.topics.map((topic, index) => (
              <Accordion
                key={index}
                title={`Topic ${index + 1} - ${topic}`}
                content={summary?.explain[index] || "No explanation available"}
